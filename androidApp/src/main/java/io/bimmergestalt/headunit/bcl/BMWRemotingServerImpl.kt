...
class BMWRemotingServerImpl(val client: BMWRemotingClient,
	...
	val rhmiResources = HashMap<BMWRemoting.RHMIResourceType, ArrayList<ByteArray>>()
	...
	override fun rhmi_setResource(
		...
		Log.i(TAG, "rhmi_setResources type:$type")
		if (rhmiHandle == null || rhmiHandle != handle) {
			throw BMWRemoting.IllegalArgumentException(-1, "Incorrect RHMI handle")
		}
		type ?: return
		data ?: return
		rhmiResources.getOrPut(type, {ArrayList()}).add(data)
		Log.i(TAG, "rhmi_setResources type:$type success $rhmiResources")
	}
